...
int main(void)
{
	...
	int rc;		// Process ID of child
	int status;	// Integer variable to record status of child process
	int result;	// Allows for error checking of PID
	bgrnd = 0;
	...
	if(rc<0){	//fork failed
		...
		if (execvp(args[0], args) == -1) {
        		perror("execvp");
        		exit(EXIT_FAILURE);  // If execvp fails, exit the child process with an error code
    		}
	}
	else if(bgrnd == 0){	//command input was not followed by '&'
		result = waitpid(rc, &status, 0);
		if(result ==-1){
			perror("waitpid");
		}
	}
